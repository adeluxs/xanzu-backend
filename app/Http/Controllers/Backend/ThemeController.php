...
class ThemeController extends Controller
{
    ...
    public function statusUpdate(Request $request)
    {
        ...
        $status = $theme->type == 'site' ? true : $input['status'];
    ...
    public function dynamicLandingUpdate(Request $request)
    {
        ...
        if ($zip->open($zipThemeFile) !== true) {
            ...
            notify()->error(__('Failed to open the zip file'), 'Error');
        ...
        if (! $indexHtmlExists) {
            ...
            notify()->error(__('The zip file does not contain index.html'), 'Error');
