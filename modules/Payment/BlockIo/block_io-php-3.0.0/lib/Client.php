...
class Client
{
    ...
    { // method_missing for PHP
        ...
        if (in_array($name, $this->sweep_methods)) { // it is a sweep method
            $response = $this->_internal_prepare_sweep_transaction($name, $args);
        } else { // pass-through method to Block.io
     ...
     */
    private function _request($path, $args = [])
    {
        // Generate cURL URL
    ...
    public function create_and_sign_transaction($data, $keys = [])
    {
        ...
        if (! is_null($this->pin)) {
            ...
            if (property_exists($data->data, 'user_key') && ! array_key_exists($data->data->user_key->public_key, $this->userKeys)) {
                ...
                if ($key->getPublicKey() != $data->data->user_key->public_key) {
        ...
        foreach ($inputs as &$curInput) {
            ...
            $curPubKeyIndex = 0;
            $ecAdapter = Bitcoin::getEcAdapter(); // we need this to add our own low-S and low-R signatures
            ...
            foreach ($curPubKeys as &$curPubKey) {
                if (array_key_exists($curPubKey, $this->userKeys)) {
                    ...
                    $ecadapter_signature_signature = new Signature($ecAdapter, gmp_init($points['R'], 16), gmp_init($points['S'], 16));
                    ...
                    if ($signerInput->step(0) instanceof CheckSig) {
                        ...
                        if ($signerInput->step(0)->getType() === ScriptType::P2PKH) {
                        ...
                        array_push($signatures,
                            ['input_index' => $curInput->input_index,
        ...
        $response = ['tx_type' => $data->data->tx_type, 'tx_hex' => null, 'signatures' => null];
    ...
    private function _internal_prepare_sweep_transaction($name, $args = [])
    ...
    { // returns the appropriate BitWasp Network object
        ...
        if ($n == 'BTCTEST') {
            $nf = $nf->bitcoinTestnet();
        } elseif ($n == 'LTCTEST') {
            $nf = $nf->litecoinTestnet();
        } elseif ($n == 'DOGETEST') {
            $nf = $nf->dogecoinTestnet();
        } elseif ($n == 'BTC') {
            $nf = $nf->bitcoin();
        } elseif ($n == 'LTC') {
            $nf = $nf->litecoin();
            ...
            throw new \Exception('Invalid network found: '.$n);
        }
