<form action="{{ route('admin.navigation.menu.update') }}" method="post">
    ...
    <div class="site-input-groups">
        ...
        <select name="type[]" multiple class="form-select type displayInSelect">
            <option value="{{ \App\Enums\NavigationType::Header->value }}" @selected(in_array(\App\Enums\NavigationType::Header->value, $navigation->type, true))>{{ __('Header') }}</option>
            <option value="{{ \App\Enums\NavigationType::FooterWidget1->value }}" @selected(in_array(\App\Enums\NavigationType::FooterWidget1->value, $navigation->type, true))>{{ __('Footer Widget 1') }}</option>
        </select>
    </div>
