                        ...
                        <div class="col-xl-8 col-lg-8 col-md-9 col-12">
                            <div class="wrap-custom-file {{ $errors->has($field['name']) ? 'has-error' : '' }}">
                                <input type="{{ $field['type'] }}" name="{{ $field['name'] }}" id="{{ $field['name'] }}"
                                    value="{{ oldSetting($field['name'], $section) }}" accept=".jpeg, .jpg, .png" />
                    ...
                    <div class="site-input-groups row">
                        ...
                        <div class="col-sm-8">
                            <select name="{{ $field['name'] }}"
                                class="form-select {{ $errors->has($field['name']) ? 'has-error' : '' }}">
                                @foreach (\App\Models\Plugin::where('type', 'ai')->where('status', 1)->get() as $option)
                                    <option value="{{ $option['name'] }}"
                                        {{ oldSetting($field['name'], $section) == $option['name'] ? 'selected' : '' }}>
